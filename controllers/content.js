...
module.exports = {
  get: async (req, res) => {
    ...
    if (contentId) {
      ...
      if (getContent.Comments.length !== 0) {
        // ! 댓글 작성일
        const commentCreatedAtArr = getContent.Comments.map(
          (el) => el.createdAt
        );
        // ! 댓글 내용
        ...
        // ! 댓글 작성자 찾기
        const commentUserIdArr = getContent.dataValues.Comments.map(
          (el) => el.dataValues.userId
        );
        ...
        // ! 댓글 작성자 순서대로 배열에 담음
        const getCommentUserName = commentUser.map(
          (el) => el.dataValues.User.dataValues.name
        );
